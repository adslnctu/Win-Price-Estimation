...
def EM_censored_gamma(thetaName, Trainday, iterations, eta1, eta2, trainY, trainP, trainLabel, n):	
	...
	for i in xrange(0, n):
		if trainLabel[i]=="win":
			 price += trainY[i]
		elif trainLabel[i]== "lose":
			price += trainP[i]
	meanOfAll = 1.0*price/n
	varWLTheta = 0
	for i in xrange(0, n):
		if trainLabel[i]=="win":
			varWLTheta += pow(trainY[i], 2)
		elif trainLabel[i]== "lose":
			varWLTheta += pow(trainP[i], 2)
	b = 1.0*varWLTheta2/n
...
def main():
	trainList = ["19", "20", "21", "22", "23", "24", "25", "26"]
	testList  = [ "20", "21", "22", "23", "24", "25", "26", "27"]
	iterations = 100
	...
	for i in xrange(0, len(trainList)):
		...
		trainName = "../../../../log/season3/simulated_1_6/2013-10-"+trainList[i]+".pickle"
		trainX, trainY, trainP, trainLabel, n = LoadData(trainName)

		#import testing data
		testName = "../../../../log/season3/simulated_1_6/2013-10-"+testList[i]+".pickle"
		testX, testY, testingB, testlabel, testn = LoadData(testName)

		ofile = "./Log/mseLog/"+str(int(trainList[i]))+"_RidgeVarWL.log"
		f = open(ofile, "a")
